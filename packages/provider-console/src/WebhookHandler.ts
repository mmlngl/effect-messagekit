...
export interface ConsoleWebhookHandlerParams<E = never, R = never> {
  payload: Events.ConsoleWebhookBodyType;
  onEvents: (
    events: ReadonlyArray<Events.WebhookEventType>,
  ) => Effect.Effect<
    ReadonlyArray<Option.Option<Messages.AnyOutboundConsoleMessageType>>,
    E,
    R
  >;
}

export type ConsoleWebhookHandler<E = never, R = never> = (
  params: ConsoleWebhookHandlerParams<E, R>,
) => Effect.Effect<P.HttpServerResponse.HttpServerResponse, E, R>;

const groupByRecipient = <T extends Messages.AnyOutboundConsoleMessageType>(
...
export const make = <E = never, R = never>({
  payload,
  onEvents,
}: ConsoleWebhookHandlerParams<E, R>) =>
  Effect.gen(function* () {
    const client = yield* Client.ConsoleClient;
    ...
    yield* Effect.forEach(
      ...
      ([recipient, outboundMessages]) =>
        client
          ...
          .pipe(
            Effect.mapError((cause) => new Errors.ConsoleApiError({ cause })),
          ),
      {
        concurrency: "inherit",
        discard: true,
